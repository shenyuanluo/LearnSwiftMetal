...
class ViewController: UIViewController {
    ...
    private var lookupTableTexture: MTLTexture!                 // 查表-纹理对象
    ...
    // MARK: 设置查表-纹理
    private func setupLookupTableTexture() {
        guard let path = Bundle.main.path(forResource: "LearnSwiftMetal", ofType: "bundle"),
              let bundle     = Bundle(path: path),
              let file       = bundle.path(forResource: "LookupTable", ofType: "png") else {
            print("Can not find normal image file.")
            return
        }
        guard let img = UIImage(contentsOfFile: file) else {
        ...
        descriptor.height       = Int(img.size.height)   // 纹理-高度
        descriptor.usage        = .shaderRead
        self.lookupTableTexture = self.device.makeTexture(descriptor: descriptor)    // 创建纹理
        // 纹理上传范围
        let region = MTLRegion(origin: MTLOrigin(x: 0, y: 0, z: 0),
                               size: MTLSize(width: Int(img.size.width), height: Int(img.size.height), depth: 1))
        // UIImage 的数据需要转成二进制才能上传，且不用 jpg、png 的 NSData
        ...
        // 拷贝图像数据到纹理
        self.lookupTableTexture.replace(region: region,
                                        mipmapLevel: 0,
                                        withBytes: data,
                                        bytesPerRow: Int(img.size.width) * 4)
    }
    ...
    func draw(in view: MTKView) {
        ...
        if let renderEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: descriptor) {
            ...
            renderEncoder.setFragmentTexture(self.normalTexture,
                                             index: Int(SYFragmentTextureIndexNormal.rawValue))
            // 设置查表-纹理
            renderEncoder.setFragmentTexture(self.lookupTableTexture,
                                             index: Int(SYFragmentTextureIndexLookupTable.rawValue))
            // 开始绘制
