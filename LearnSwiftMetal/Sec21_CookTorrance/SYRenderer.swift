...
//        (modelIOVertexDescriptor.attributes[4] as! MDLVertexAttribute).name = MDLVertexAttributeBitangent
    ...
    private func updateGameState() {
        ...
        let viewMatrix  = moveToZ * rotateX
        ...
        uniforms.pointee.modelViewMatrix  = viewMatrix * modelMatrix
        ...
        uniforms.pointee.Ks        = 0.9    // 镜面反射系数
        uniforms.pointee.Ia        = 3.0    // 环境光强度
        uniforms.pointee.Ka        = 0.1    // 环境光系数
//        uniforms.pointee.shininess = 15     // 镜面反射强度（高光指数）
        uniforms.pointee.f         = 0.015  // 菲涅尔系数
        uniforms.pointee.m         = 0.8    // 法线分布粗糙度
        uniforms.pointee.cameraPos = vector_float3(0, 100, -1100)   // 相机位置（世界空间）
